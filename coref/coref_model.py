...
class CorefModel:  # pylint: disable=too-many-instance-attributes
    ...
    def evaluate(self,
        ...
        with conll.open_(self.config, self.epochs_trained, data_split) \
            ...
            for doc in pbar:
                doc1 = doc
                # doc = doc_from_text(doc["cased_words"], doc["sent_id"], doc["part_id"],doc["document_id"])
                # doc = self._tokenize_docs_talisman(doc)
                res = self.run(doc)
                ...
                if word_level_conll:
                    conll.write_conll(doc,
                                      [[(i, i + 1) for i in cluster]
                                       for cluster in doc1["word_clusters"]],
                                      gold_f)
                    conll.write_conll(doc,
                                      [[(i, i + 1) for i in cluster]
                    ...
                    conll.write_conll(doc, doc1["span_clusters"], gold_f)
                    conll.write_conll(doc, res.span_clusters, pred_f)
                w_checker.add_predictions(doc1["word_clusters"], res.word_clusters)
                w_lea = w_checker.total_lea

                s_checker.add_predictions(doc1["span_clusters"], res.span_clusters)
        ...
        with jsonlines.open(path, mode="r") as data_f:
            for doc in data_f:
                ...
                for i, word in enumerate(doc["cased_words"]):
                    ...
                    # boundaries of words in the token list
    ...
    def _tokenize_docs_talisman(self, doc) -> List[Doc]:
        filter_func = TOKENIZER_FILTERS.get(self.config.bert_model,
                                        lambda _: True)
        token_map = TOKENIZER_MAPS.get(self.config.bert_model, {})
        word2subword = []
